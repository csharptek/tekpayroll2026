...
import { computeSalary, ri, r2, DEFAULT_ESI_EMPLOYEE_RATE, DEFAULT_ESI_EMPLOYER_RATE, DEFAULT_ESI_THRESHOLD } from '../utils/salaryFormula'
...
export default function SalaryCalculatorForm({ onChange, initialValues, showInstructions = true }: Props) {
  ...
  function runCompute(c: number, bp: number, hp: number, ip: number, hi: boolean, m: number) {
    return computeSalary({
      ctc: c, basicPct: bp, hraPct: hp, incentivePct: ip, hasIncentive: hi, mediclaim: m,
      esiEmployeeRate, esiEmployerRate, esiThreshold,
    })
  }
  ...
  const [components,  setComponents]  = useState<Components>(() => {
    if ((initialValues?.annualCtc ?? 0) > 0) {
      ...
      const c  = runCompute(iv.annualCtc!, iv.basicPercent ?? 45, iv.hraPercent ?? 35, iv.incentivePercent ?? 12, iv.hasIncentive ?? false, iv.mediclaim ?? 0)
      return { basic: c.basic, hra: c.hra, transport: c.transport, fbp: c.fbp, hyi: c.hyi }
    }
  ...
  const [grandTotal,  setGrandTotal]  = useState(() => {
    if ((initialValues?.annualCtc ?? 0) > 0) {
      const iv = initialValues!
      return runCompute(iv.annualCtc!, iv.basicPercent ?? 45, iv.hraPercent ?? 35, iv.incentivePercent ?? 12, iv.hasIncentive ?? false, iv.mediclaim ?? 0).grandTotal
    }
    return 0
  })
  ...
  // Derived
  const employerPf    = Math.min(ri(components.basic * 0.12), 1800)
  ...
  const esiApplies    = initialized && components.basic > 0 && components.basic < esiThreshold
  const employeeEsi   = esiApplies ? ri(components.basic * esiEmployeeRate) : 0
  const employerEsi   = esiApplies ? ri(components.basic * esiEmployerRate) : 0
  ...
  function applyCtc() {
    if (ctc <= 0) return
    const c = runCompute(ctc, basicPct, hraPct, incentivePct, hasIncentive, mediclaim)
      ...
      {showInstructions && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs font-semibold text-amber-700 uppercase tracking-wide">Salary Formula (New ESIC)</p>
          </div>
          <ul className="space-y-1 text-xs text-amber-700 leading-relaxed">
            ...
            <li>&#8226; Employee PF &amp; Employee ESI are <strong>inside Gross</strong> (deducted)</li>
            <li>&#8226; Grand Monthly = (CTC &#8722; Mediclaim) &#247; 12 &#8722; Bonus/12 &#8722; Employer PF &#8722; Employer ESI</li>
            <li>&#8226; Transport = 2% of Gross, FBP = 2% of Gross, HYI = balancer</li>
          </ul>
        </div>
      )}
