...
function computeFromCtc(ctc: number, basicPct: number, hraPct: number, incentivePct: number, hasIncentive: boolean, mediclaim: number) {
  ...
  const employerPf      = ri(basicMonthly * 0.12)
  return { basic: basicMonthly, hra: hraMonthly, transport, fbp, hyi, grandTotal, employerPfInCtc, employerPf, annualBonus }
}
...
export default function SalaryCalculatorNewEsicPage() {
  ...
  // NEW ESI LOGIC: based on basic < threshold
  const esiBase     = components.basic
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">ESI applies when Basic &lt; &#8377;{esiThreshold.toLocaleString('en-IN')} — new rule</p>
        </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-xs font-semibold text-amber-700 uppercase tracking-wide">New ESIC Rule</p>
        </div>
        <ul className="space-y-1 text-xs text-amber-700 leading-relaxed">
          <li>&#8226; ESI applies when <strong>Basic &lt; &#8377;{esiThreshold.toLocaleString('en-IN')}</strong> (not Gross &#8722; HYI)</li>
          <li>&#8226; Employee ESI = {(esiEmployeeRate * 100).toFixed(2)}% of Basic</li>
          <li>&#8226; Employer ESI = {(esiEmployerRate * 100).toFixed(2)}% of Basic (outside CTC)</li>
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="bg-red-50/50 border border-red-100 rounded-xl p-3">
              ...
              <div className="space-y-1 text-xs text-slate-600">
                ...
                {esiApplies ? (
                  <div className="flex justify-between">
                    <span>Employee ESI ({(esiEmployeeRate * 100).toFixed(2)}% of Basic = <Rupee amount={esiBase}/>)</span>
