  ...
  return (
    <Card className="p-5">
      ...
      <div className="flex items-center gap-2">
        ...
        <Button
          size="sm"
          onClick={() => navigate(`/hr/payroll/${cycle.id}/run`)}
      ...
      <div className="grid grid-cols-2 gap-2 p-5 pt-0">
        {actions.map(({ label, icon: Icon, color, to }) => (
          <button
            key={label}
            onClick={() => navigate(to)}
            className={`flex items-center gap-3 p-3 rounded-xl transition-all duration-150 text-left group ${color}`}
          >
            <Icon size={16} />
...
function AlertsCard() {
  // These would be real data in production
  const alerts = [
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Net Payout"
          value={latestCycle?.totalNet
            ? `₹${Number(latestCycle.totalNet).toLocaleString('en-IN')}`
            : '—'}
        ...
        <StatCard
          label="Total PF"
          value={latestCycle?.totalPf
            ? `₹${Number(latestCycle.totalPf).toLocaleString('en-IN')}`
            : '—'}
          sub="This cycle"
