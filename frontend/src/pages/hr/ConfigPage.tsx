...
import { Save, Plus, Trash2, Settings, Shield, Calendar, DollarSign } from 'lucide-react'
...
export default function ConfigPage() {
  ...
  const { data: config, isLoading: loadingConfig } = useQuery({
    ...
    onSuccess: (data: any) => setConfigValues(data),
  })

  const { data: slabs, isLoading: loadingSlabs } = useQuery({
    queryKey: ['pt-slabs'],
    queryFn: () => configApi.ptSlabs().then(r => r.data.data),
    onSuccess: (data: any[]) => setPtSlabs(data),
  })
  ...
  function getValue(key: string, dflt: string) {
    return configValues[key] ?? config?.[key] ?? dflt
  }
  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      <Section icon={Shield} title="Professional Tax Slabs">
        <p className="text-xs text-slate-400 mb-4">Configure state-wise PT slabs. The correct slab is picked based on each employee's state.</p>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <Input type="number" placeholder="Max Salary (blank = no limit)" value={newSlab.maxSalary} onChange={e => setNewSlab(p => ({ ...p, maxSalary: e.target.value }))} />
              ...
              <div className="mt-3 flex justify-end">
                <Button size="sm" icon={<Plus size={13} />}
                  disabled={!newSlab.state || !newSlab.ptAmount}
                  onClick={() => {
                    setPtSlabs(p => [...p, { id: Date.now(), ...newSlab }])
                    setNewSlab({ state: '', minSalary: '', maxSalary: '', ptAmount: '' })
                  }}>
                  Add Slab
                </Button>
              </div>
